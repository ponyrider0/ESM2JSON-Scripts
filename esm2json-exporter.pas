...
function FormatNativeValue2(native_value: Variant): string;
var
...
begin
  ...
  if ( (native_type = 0) or (native_type = 1) ) then
  begin
    element_edit_value := '';
  end
  else if ( (native_type = 2) or (native_type = 3) ) then
  begin
    element_edit_value := IntToStr(native_value);
  end
  else if (native_type = 5) then
  begin
    element_edit_value := FloatToStrF(native_value, 2, 15, 15);
  end
  else if (native_type = 11) then
  begin
    if (native_value = True) then
      element_edit_value := 'true'
    else
      element_edit_value := false;
  end
  ...
  else if (native_type = 18) then
  begin
    element_edit_value := '"' + IntToHex(native_value, 4) + 'H"';
  end
  else if (native_type = 19) then
  begin
    element_edit_value := '"' + IntToHex(native_value, 8) + 'H"';
  end
  else if (native_type = 258) then
  begin
    element_edit_value := StringReplace(native_value,'\','\\', [rfReplaceAll]);
    element_edit_value := StringReplace(element_edit_value,'"','\"', [rfReplaceAll]);
    element_edit_value := StringReplace(element_edit_value, #13#10, '\r\n', [rfReplaceAll]);
  ...
  begin
    AddMessage('ERROR: Unknown native_type=' + IntToStr(native_type) );
  end;
...
function ProcessRecord_Step4(e: IInterface; prefix: string=''; postfix: string='' ): integer;
var
  child_element, element: IInterface;
  native_type, element_count, element_index, child_count: integer;
  element_name, element_path, element_edit_value, prefix2, postfix2, postfix3: string;
  native_value: Variant;
  parent_type, element_type: TwbElementType;
  array_count, i, stringlist_length: integer;
  index_string, type_string: string;
begin
  ...
  begin
    ...
    begin
      ...
      if (child_count = 0) then
      begin
        if ((element_type = etArray) Or (element_type = etSubRecordArray)) then
          element_edit_value := '[]'
        else
          element_edit_value := '{}';
      end
    ...
    type_string := '{' + IntToStr(element_type) + '}';
    json_output.append(prefix + prefix2 + '"' + index_string + type_string + element_name + '": ' + element_edit_value + postfix2);
    ...
    begin
      ...
      begin
        ...
        AddMessage('ERROR: recursion deactivated');
//      ProcessRecord_Step2(element, prefix + prefix2, postfix2);

      if ((element_type = etArray) Or (element_type = etSubRecordArray)) then
        json_output.append(prefix + prefix2 + ']' + postfix2 + postfix3)
      else
        json_output.append(prefix + prefix2 + '}' + postfix2 + postfix3);
    end;
...
function ProcessRecord_Step3(e: IInterface; prefix: string=''; postfix: string='' ): integer;
var
  child_element, element: IInterface;
  native_type, element_count, element_index, child_count: integer;
  element_name, element_path, element_edit_value, prefix2, postfix2, postfix3: string;
...
//      AddMessage('ERROR: recursion deactivated');
      ProcessRecord_Step4(element, prefix + prefix2, postfix2);
...
function ProcessRecord_Step2(e: IInterface; prefix: string=''; postfix: string='' ): integer;
var
  child_element, element: IInterface;
  native_type, element_count, element_index, child_count: integer;
  element_name, element_path, element_edit_value, prefix2, postfix2, postfix3: string;
...
//      AddMessage('ERROR: recursion deactivated');
      ProcessRecord_Step3(element, prefix + prefix2, postfix2);
...
function ProcessRecord_Step0(e: IInterface; prefix: string=''; postfix: string='' ): integer;
var
  child_element, element: IInterface;
  native_type, element_count, element_index, child_count: integer;
  element_name, element_path, element_edit_value, prefix2, postfix2, postfix3: string;
...
//      AddMessage('ERROR: recursion deactivated');
      ProcessRecord_Step0(element, prefix + prefix2, postfix2);
...
function ProcessRecord_Step1(e: IInterface ): integer;
var
  child_element, element: IInterface;
  native_type, element_count, element_index, child_count: integer;
  element_name, element_path, element_edit_value, prefix, postfix, prefix2, postfix2, postfix3: string;
  native_value: Variant;
  parent_type, element_type: TwbElementType;
  array_count, i, stringlist_length: integer;
  index_string, type_string: string;
